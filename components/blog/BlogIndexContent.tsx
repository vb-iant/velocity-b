...
export function BlogIndexContent({
  ...
  // The page number in the URL path only has meaning relative to the unfiltered
  // list (that's what generateStaticParams pre-builds) — a wildly out-of-range
  // page is a real 404, not just an empty filtered result.
  const unfilteredTotalPages = Math.max(1, Math.ceil(allPosts.length / BLOG_PAGE_SIZE));
  if (currentPage > unfilteredTotalPages || currentPage < 1) {
    notFound();
  }

  const activeTag = tagSlug ? getTagBySlug(tagSlug) : null;
  ...
  return (
    <main>
      ...
      {/* Tag filter — functional, layered on top of path-based pagination via ?tag= */}
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <Link
                ...
                className={`flex h-9 w-9 items-center justify-center rounded-full font-display text-sm font-bold ${
                  p === pageForSlice ? "bg-navy text-white" : "text-[#42465c] hover:text-blue"
                }`}
              >
                {p}
              </Link>
            ))}
            {pageForSlice < totalPages && (
              <Link
                href={pageHref(pageForSlice + 1)}
